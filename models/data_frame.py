...
class DataFrame(object):
    ...
    def train_test_split(self, train_size, group_col=None, random_state=42):
        if group_col is not None and group_col in self.columns:
            groups = self[group_col]
            unique_groups = np.unique(groups)
            train_groups, test_groups = train_test_split(unique_groups, train_size=train_size, random_state=random_state)
            train_mask = np.isin(groups, train_groups)
            test_mask = np.isin(groups, test_groups)
            train_df = DataFrame(copy.copy(self.columns), [mat[train_mask] for mat in self.data])
            test_df = DataFrame(copy.copy(self.columns), [mat[test_mask] for mat in self.data])
