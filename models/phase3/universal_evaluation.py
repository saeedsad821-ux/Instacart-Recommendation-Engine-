...
def generate_universal_reports(root_dir="."):
    print("=========================================================================")
    print("      PHASE 3 — GENERATING UNIVERSAL GROUND-TRUTH REPORTS                ")
    print("=========================================================================")
    
    out_dir = os.path.join(root_dir, "artifacts/reports")
    os.makedirs(out_dir, exist_ok=True)
    
    # 1. Write phase3_data_usage_audit.md
    data_audit_md = """# Phase 3 — Forensic Data Usage Audit Report
...
"""
    with open(os.path.join(out_dir, "phase3_data_usage_audit.md"), "w", encoding="utf-8") as f:
        f.write(data_audit_md)
    print(f"[SUCCESS] Written {os.path.join(out_dir, 'phase3_data_usage_audit.md')}")

    # 2. Write authoritative final_recommendation_system_ground_truth.md
    master_md = """# Final Recommendation System Ground-Truth Audit, Temporal Validation & Intelligent Model Improvement Report
...
## 7. Temporal Protocol (Orders $1 \dots N-1 \rightarrow N$)
...
"""
    with open(os.path.join(out_dir, "final_recommendation_system_ground_truth.md"), "w", encoding="utf-8") as f:
        f.write(master_md)
    print(f"[SUCCESS] Written authoritative ground-truth report to {os.path.join(out_dir, 'final_recommendation_system_ground_truth.md')}")
