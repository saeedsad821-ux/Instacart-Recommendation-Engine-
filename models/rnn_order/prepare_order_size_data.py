...
if __name__ == '__main__':
    ...
    for idx, row in user_data.iterrows():
        ...
        products = str(row['product_ids'])
        products, next_products = ' '.join(products.split()[:-1]), products.split()[-1]

        reorders = str(row['reorders'])
        reorders, next_reorders = ' '.join(reorders.split()[:-1]), reorders.split()[-1]

        orders = [[int(x) for x in i.split('_')] for i in products.split()]
        reorders = [[int(x) for x in i.split('_')] for i in reorders.split()]

        next_reorders = [int(x) for x in next_reorders.split('_')]
        next_orders = [int(x) for x in next_products.split('_')]
        ...
        order_number_history[idx, :], _ = pad_1d([int(x) for x in str(row['order_numbers']).split()[:-1]], 100)
        order_dow_history[idx, :], _ = pad_1d([int(x) for x in str(row['order_dows']).split()[:-1]], 100)
        order_hour_history[idx, :], _ = pad_1d([int(x) for x in str(row['order_hours']).split()[:-1]], 100)
        days_since_prior_order_history[idx, :], _ = pad_1d([int(x) for x in str(row['days_since_prior_orders']).split()[:-1]], 100)
