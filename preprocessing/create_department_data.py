...
if __name__ == '__main__':
    ...
    print(f"[{time.ctime()}] Starting create_department_data.py (Memory: {get_memory_mb():.1f} MB)", flush=True)
    ...
    for row in df.itertuples(index=False):
        ...
        depts_split = str(row.department_ids).split()
        reords_split = str(row.reorders).split()
        
        depts_hist = depts_split[:-1]
        next_depts_str = depts_split[-1] if len(depts_split) > 0 else ""
        next_depts_set = set(next_depts_str.split('_')) if next_depts_str else set()
        ...
        order_maps = []
        order_sizes_list = []
        for o_str in depts_hist:
            items = o_str.split('_')
            order_sizes_list.append(str(len(items)))
            o_set = set(items)
            o_map = {d: str(idx + 1) for idx, d in enumerate(items)}
        ...
        for o_set in order_sets:
            dept_set.update(o_set)

        for d_id_str in dept_set:
            is_ord = ' '.join('1' if d_id_str in o_set else '0' for o_set in order_sets)
            idx_in_ord = ' '.join(o_map.get(d_id_str, '0') for o_map in order_maps)
            lbl = 1 if d_id_str in next_depts_set else 0

            out_user_id.append(u_id)
            out_department_id.append(int(d_id_str))
        ...
        # Add None department (0)
        zeros_str = ' '.join(['0'] * len(order_sets))
        none_lbl = 1 if len(next_depts_set) == 0 else 0
        out_user_id.append(u_id)
        out_department_id.append(0)
    ...
    out_df = pd.DataFrame({
        'user_id': out_user_id,
        'department_id': out_department_id,
    ...
    print("Merging with departments.csv metadata...", flush=True)
    departments = pd.read_csv('../data/raw/departments.csv')
    none_row = pd.DataFrame({'department_id': [0], 'department': ['none']})
    departments = pd.concat([departments, none_row], ignore_index=True)
    out_df = out_df.merge(departments[['department_id', 'department']], on='department_id', how='left')
    out_df['department'] = out_df['department'].fillna('none')
    ...
    output_path = '../data/processed/department_data.csv'
