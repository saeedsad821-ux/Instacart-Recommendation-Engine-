...
if __name__ == '__main__':
    ...
    print(f"[{time.ctime()}] Starting create_aisle_data.py (Memory: {get_memory_mb():.1f} MB)", flush=True)
    ...
    for row in df.itertuples(index=False):
        ...
        aisles_split = str(row.aisle_ids).split()
        reords_split = str(row.reorders).split()
        
        aisles_hist = aisles_split[:-1]
        next_aisles_str = aisles_split[-1] if len(aisles_split) > 0 else ""
        next_aisles_set = set(next_aisles_str.split('_')) if next_aisles_str else set()
        ...
        order_maps = []
        order_sizes_list = []
        for o_str in aisles_hist:
            items = o_str.split('_')
            order_sizes_list.append(str(len(items)))
            o_set = set(items)
            o_map = {a: str(idx + 1) for idx, a in enumerate(items)}
        ...
        for o_set in order_sets:
            aisle_set.update(o_set)

        for a_id_str in aisle_set:
            is_ord = ' '.join('1' if a_id_str in o_set else '0' for o_set in order_sets)
            idx_in_ord = ' '.join(o_map.get(a_id_str, '0') for o_map in order_maps)
            lbl = 1 if a_id_str in next_aisles_set else 0

            out_user_id.append(u_id)
            out_aisle_id.append(int(a_id_str))
        ...
        none_lbl = 1 if len(next_aisles_set) == 0 else 0
        out_user_id.append(u_id)
        out_aisle_id.append(0)
    ...
    print("Merging with aisles.csv metadata...", flush=True)
    aisles = pd.read_csv('../data/raw/aisles.csv')
    none_row = pd.DataFrame({'aisle_id': [0], 'aisle': ['none']})
    aisles = pd.concat([aisles, none_row], ignore_index=True)
    out_df = out_df.merge(aisles[['aisle_id', 'aisle']], on='aisle_id', how='left')
    out_df['aisle'] = out_df['aisle'].fillna('none')
    ...
    output_path = '../data/processed/aisle_data.csv'
